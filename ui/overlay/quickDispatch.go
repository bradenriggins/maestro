...
func (q *QuickDispatchOverlay) HandleKeyPress(msg tea.KeyMsg) (shouldClose bool) {
	...
	case tea.KeyEsc:
		...
		return true
	case tea.KeyEnter:
		if strings.TrimSpace(q.taskInput) != "" && len(q.workers) > 0 {
			q.submitted = true
			return true
		}
		return false
	...
	case tea.KeyBackspace:
		if len(q.taskInput) > 0 {
			q.taskInput = q.taskInput[:len(q.taskInput)-1]
		}
		return false
	case tea.KeyRunes:
...
func (q *QuickDispatchOverlay) Render() string {
	...
	if len(q.workers) == 0 {
		...
		for i, w := range q.workers {
			if i == q.workerIdx {
				b.WriteString(qdSelectedStyle.Render("[" + w + "]"))
			} else {
				b.WriteString(qdDimStyle.Render(" " + w + " "))
			}
